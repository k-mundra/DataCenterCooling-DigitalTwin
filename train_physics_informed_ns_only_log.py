...
#
# Modified for NS-only physics-informed training with every-step console logging.
...
def main(cfg: DictConfig) -> None:
    ...
    save_every = cfg.get("save_every", cfg.get("save_every_epochs", 2))
    ...
    nu = cfg.get("nu", 0.01)
    rho = cfg.get("rho", 1.0)
    temp_source = cfg.get("temp_source", 0.0)  # unused in NS-only training
    checkpoint_dir = cfg.get("checkpoint_dir", "/home/iaziz6/Imo_Training/Training/checkpoints_physics_ns_only")
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.start_lr, betas=(0.9, 0.999), weight_decay=0.0, fused=True)
    ...
    def _save_loss_curve() -> None:
        ...
        ax.set_xlabel("Step")
        ax.set_ylabel("Loss")
        ax.set_title("NS-Only Physics-Informed Training Curve")
        ax.legend()
        fig.tight_layout()
    ...
    logger.info(
        f"Using NS-only physics-informed training with train_num_samples={train_num_samples}, "
        f"val_num_samples={val_num_samples}, max_epochs={max_epochs}, "
        f"log_every_steps={log_every}, val_every_steps={val_every}, save_every={save_every}, checkpoint_dir={checkpoint_dir}."
    )
    ...
    for epoch in range(max(1, loaded_epoch + 1), max_epochs + 1):
        with LaunchLogger("train", epoch=epoch, num_mini_batch=len(dataset), epoch_alert_freq=1) as log:
            for step_in_epoch, data in enumerate(dataset, 1):
                ...
                # T is still predicted as part of the data loss, but no temperature PDE loss is used in NS-only training.
                ...
                # NS-only physics-informed loss.
                # No temperature/energy-equation residual is included here.
                loss = data_loss + phy_wt * ns_phy_loss
                loss.backward()
                optimizer.step()
                scheduler.step()
