...
def main(cfg: DictConfig) -> None:
    ...
    num_ensembles = 5  # Number of ensembles being used for inference
    ...
    for design in combinations:
        ...
        for ensemble_id in range(num_ensembles):
            ...
            checkpoint_dir = f"./outputs/checkpoints/ensemble_{ensemble_id}"
            ...
            with torch.no_grad():
                ensemble_preds.append(model(invar_tensor))

            ## Clear storage and cache before loading next model
            del model
            gc.collect()
        ...
        pred_var = ensemble_preds.var(dim=0)
        ...
        var = {
            "u_x_pred": pred_mean_np[0, 0],
            "u_x_std": np.sqrt(pred_var_np[0, 0]),
            "u_y_pred": pred_mean_np[0, 1],
            "u_y_std": np.sqrt(pred_var_np[0, 1]),
            "u_z_pred": pred_mean_np[0, 2],
            "u_z_std": np.sqrt(pred_var_np[0, 2]),
            "T_pred": pred_mean_np[0, 3],
            "T_std": np.sqrt(pred_var_np[0, 3]),
            "p_pred": pred_mean_np[0, 4],
            "p_std": np.sqrt(pred_var_np[0, 4]),
